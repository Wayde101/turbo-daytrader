                                     ...
                                     const char* hostname)
{
    ...
    if (count > 0) {
        ...
        for (i=1; i<count; ++i)
            pcre_get_substring(hostname, offsets, count, i, &buf[i-1]);
        rangeparts->prefix = apr_pstrdup(pool, buf[0]);
        rangeparts->first = apr_pstrdup(pool, buf[1]);
        rangeparts->last = apr_pstrdup(pool, buf[3]);
        if(strcmp(buf[2], "")) rangeparts->domain = apr_pstrdup(pool, buf[2]);
        else rangeparts->domain = apr_pstrdup(pool, buf[4]);

        for (i=0; i<(count-1); ++i) pcre_free_substring(buf[i]);
        return rangeparts;
    }
