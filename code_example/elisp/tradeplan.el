...
(defvar widget-demo-buffer-name "*每日功课*"
  "*Name of the widget demo buffer")

(defvar widget-demo-list
  '(("Contents" widget-tp-contents :header "每日功课")
    ("Example" widget-tp-usdx :header "市场整体情况")
...
(defvar widget-demo-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "u" 'widget-tradeplan)
    (define-key map "t" 'widget-tradeplan)
    ...
    (define-key map "r" 'widget-demo-reset)
...
(defun widget-demo-show-source ()
  ...
  (let ((page (assoc widget-demo-current widget-demo-list)))
    (with-selected-window
        (display-buffer
         (find-file-noselect (find-library-name "widget-tradeplan")))
      (imenu (symbol-name (cadr page)))
      (recenter 1))))
;;}}}

;;{{{  Pages
(defun widget-tp-contents ()
  (let ((idx 1))
...
(defun widget-tp-usdx ()
  ...
  (widget-create 'menu-choice
                 ...
                 '(choice-item "heihei option")
                 '(choice-item "haha option")
...
(defun widget-demo-reset ()
  (interactive)
  (progn (unload-feature 'widget-tradeplan) 
	 (load-file "~/org/tradeplan.el")
	 (kill-buffer "*每日功课*")
	 (widget-tradeplan))

)

(provide 'widget-tradeplan)
