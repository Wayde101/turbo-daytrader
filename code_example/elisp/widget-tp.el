...
;; Version: $Id: tp.el,v 0.0 2007/11/13 17:50:14 ywb Exp $
;; Keywords: 
...
;; creating buffer and the key bindings of `tp-mode'. Here is
...
;;   (autoload 'tp "tp" "Show demo of widget" t)
...
(defvar tp-buffer-name "*Widget Demo*"
  "*Name of the widget demo buffer")

(defvar tp-list
  '(("Contents" tp-contents :header "Widget Demo")
    ("Example" tp-example :header "Example in info")
...
(unless tp-menu
  (easy-menu-define
   tp-menu tp-mode-map "Widget Demo"
   `("Widget"
     ["Next Page" tp-next t]
     ["Previous Page" tp-previous t]
     ["Top" tp t]
     ["Refresh" tp-reflesh t]
     "--"
     ,@(mapcar (lambda (p)
                 (vector (car p) 'tp-menu-goto t))
               tp-list))))
...
(define-derived-mode tp-mode nil "WDemo"
  "Widget demo.
\\{tp-mode-map}"
...
(defun tp-example ()
  ...
  (widget-create 'push-button
                 :notify (lambda (&rest ignore)
                           (if (= (length (widget-value (tp-form-get 'repeat)))
                                  3)
...
(defun tp-button ()
  ...
  (tp-form-create
   ...
   (widget-create 'push-button
                  ...
                  :notify (lambda (wid &rest ignore)
                            (message "Got it!"))))
  (widget-apply (tp-form-get 'button) :deactivate)
  ;; image button
...
(defun tp-text ()
  ...
  (widget-create 'checkbox
                 :notify (lambda (wid &rest ignore)
                           (let ((text (tp-form-get 'text))
                                 (but (tp-form-get 'button)))
  ...
  (tp-form-create
   ...
   (widget-create 'editable-field
                  :format "Text: %v "
                  :size 20))
  (widget-apply (tp-form-get 'text) :deactivate)
  (tp-form-create
   'button
   (widget-create 'push-button
                  :notify (lambda (wid &rest ignore)
  ...
  (widget-create 'push-button
                 :format "%[%v%]\n"
                 :notify (lambda (wid &rest ignore)
                           (message "%S" (widget-value (tp-form-get 'editable-list))))
                 "See Value")
  (tp-form-create
   'editable-list
   (widget-create 'editable-list
...
(defun tp-change-text (wid id)
  (let ((other (tp-form-get id)))
    (save-excursion
